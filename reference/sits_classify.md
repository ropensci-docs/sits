...
- [`sits_classify.sits`](https://docs.ropensci.org/sits/reference/sits_classify.sits.md)
  is called when the input is a set of time series. The output is the
  same set with the additional column `predicted`.

- [`sits_classify.raster_cube`](https://docs.ropensci.org/sits/reference/sits_classify.raster_cube.md)
...
- [`sits_classify.vector_cube`](https://docs.ropensci.org/sits/reference/sits_classify.vector_cube.md)
...
1.  [`sits_cube`](https://docs.ropensci.org/sits/reference/sits_cube.md):
    selects a ARD image collection from a cloud provider.

2.  [`sits_cube_copy`](https://docs.ropensci.org/sits/reference/sits_cube_copy.md):
    copies an ARD image collection from a cloud provider to a local
    directory for faster processing.

3.  [`sits_regularize`](https://docs.ropensci.org/sits/reference/sits_regularize.md):
    create a regular data cube from an ARD image collection.

4.  [`sits_apply`](https://docs.ropensci.org/sits/reference/sits_apply.md):
    create new indices by combining bands of a regular data cube
    (optional).

5.  [`sits_get_data`](https://docs.ropensci.org/sits/reference/sits_get_data.md):
    extract time series from a regular data cube based on user-provided
    labelled samples.

6.  [`sits_train`](https://docs.ropensci.org/sits/reference/sits_train.md):
...
8.  [`sits_smooth`](https://docs.ropensci.org/sits/reference/sits_smooth.md):
    post-process a probability cube using a spatial smoother to remove
    outliers and increase spatial consistency.

9.  [`sits_label_classification`](https://docs.ropensci.org/sits/reference/sits_label_classification.md):
...
- support vector machines:
  [`sits_svm`](https://docs.ropensci.org/sits/reference/sits_svm.md);

- random forests:
  [`sits_rfor`](https://docs.ropensci.org/sits/reference/sits_rfor.md);

- extreme gradient boosting:
  [`sits_xgboost`](https://docs.ropensci.org/sits/reference/sits_xgboost.md);

- light gradient boosting:
  [`sits_lightgbm`](https://docs.ropensci.org/sits/reference/sits_lightgbm.md);

- multi-layer perceptrons:
  [`sits_mlp`](https://docs.ropensci.org/sits/reference/sits_mlp.md);

- temporal CNN:
  [`sits_tempcnn`](https://docs.ropensci.org/sits/reference/sits_tempcnn.md);

- residual network encoders:
  [`sits_resnet`](https://docs.ropensci.org/sits/reference/sits_resnet.md);

- LSTM with convolutional networks:
  [`sits_lstm_fcn`](https://docs.ropensci.org/sits/reference/sits_lstm_fcn.md);

- temporal self-attention encoders:
  [`sits_lighttae`](https://docs.ropensci.org/sits/reference/sits_lighttae.md)
  and
  [`sits_tae`](https://docs.ropensci.org/sits/reference/sits_tae.md).
